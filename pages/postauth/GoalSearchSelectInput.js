...
export default function GoalSearchSelectInput(props) {
    ...
    const setItem = (e) => {
        setSelectedItem(e)
        handleChange(e.id, props.name, props.id)
    }
    ...
    return props.items && props.handleChange && props.name && props.selected ? (
        <Combobox as="div" value={selectedItem} onChange={(e) => setItem(e)}>
            <div className="relative mt-1 px-2">
                ...
                />
                <Combobox.Button className="absolute inset-y-0 right-1 flex items-center rounded-r-md px-2 focus:outline-none">
